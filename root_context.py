    ...
    def _next_layer(self, top_layer):
        ...
        if self.config.check_ignore:
            ...
            if not ignore and client_tls:
                try:
                    client_hello = TlsClientHello.from_client_conn(
                        self.client_conn)
                except TlsProtocolException as e:
                    self.log("Cannot parse Client Hello: %s" %
                             repr(e), "error")
                else:
                    ignore = self.config.check_ignore(
                        (client_hello.client_sni, 443))
            if ignore:
        ...
        # 3. In Http Proxy mode and Upstream Proxy mode, the next layer is
        # fixed.
